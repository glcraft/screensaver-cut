...
void Scene::init(uint32_t width, uint32_t height)
{
    ...
    // TODO
    float ratio = 1.f;// static_cast<float>(m_input.getWindowData().size.x)/static_cast<float>(m_input.getWindowData().size.y);

    m_shader << gl::sl::use
        << gl::UniformStatic<float>("sizeLine", float(width)/100.f)
    ...
    m_screens.blitter.init(sampler, glm::ivec2(width, height));
    m_screens.drawer.init(sampler, glm::ivec2(width, height));
...
void Scene::update() {
    ...
    currentTime = glm::cos(realTime/currentTimeBound);
    if (timeChange>10.f)
    {
        timeChange=10.f;
        resetTime();
...
void Scene::render(bool tex) const {
    drawScene(true);
}
...
void Scene::drawScene(bool tex) const
{
...
void Scene::Screen::FBO::init(gl::Sampler::sptr sampler, glm::ivec2 size)
{
